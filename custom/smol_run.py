...
class RobotIA:
    def __init__(self, robot, pathToPolicy):
        self.robot = robot
        self.policy = SmolVLAPolicy.from_pretrained(pathToPolicy)
        self.device = get_safe_torch_device("cuda")

    def run_task(self,task,duration=40):
        ...
        while timestamp < control_time_s:
            ...
            action_values = predict_action(observation_frame,self.policy, get_safe_torch_device("cuda"),self.policy.config.use_amp,task=task,robot_type=self.robot.robot_type)
            action = {key: action_values[i].item() for i, key in enumerate(self.robot.action_features)}

            # Send the action to the self.robot
...
if __name__=="__main__":
    ...
    while timestamp < control_time_s:
        ...
        # Predict the next action with respect to the current observation
        observation_frame = build_dataset_frame(observation, prefix="observation")
        action_values = predict_action(observation_frame,policy, get_safe_torch_device("cuda"),policy.config.use_amp,task="pick the yellow screwdriver and hold it in the air",robot_type=robot.robot_type)
